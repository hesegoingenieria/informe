import sequelize from "../libs/sequelize.js";

export const find = async (fechaInicio, fechaFin, sede) => {
  const orders = await sequelize.query(`
  SELECT
    f.name AS brigada,
    e.name AS sector,
    b.cod_cuenta AS codigo_cuenta,
    b.codigo_orden AS codigo_orden,
    b.cod_punto_servicio AS punto_servicio,
    d.name AS tipo_orden,
    b.aplicacion_origen AS aplicacion_origen,
    b.tipo_punto_servicio AS tipo_punto_servicio,
    b.circuito AS circuito,
    b.tarifa AS tarifa,
    b.tipo_voltaje AS tipo_voltaje,
    b.tipo_medida AS tipo_medida,
    b.tipo_acuerdo_servicio AS tipo_acuerdo,
    b.orden_latitud AS orden_latitud,
    b.orden_longitud AS orden_longitud,
    b.punto_medida AS punto_medida,
    b.usuario_generador_orden AS usuario_generador_orden,
    a.zona AS zona,
    b.transformador AS transformador,
    b.nodo AS nodo,
    b.state_name AS state_name,
    b.city_name AS city,
    b.localidad AS localidad,
    b.sublocalidad AS sublocalidad,
    b.fecha_de_generacion AS fecha_de_generacion,
    b.descripcion AS descripcion,
    b.fecha AS fecha_i,
    b.hora_ejecucion AS hora_ejecucion,
    b.observaciones AS observaciones,
    b.observacion_cancelacion AS observacion_cancelacion,
    b.observacion_suspension AS observacion_suspension,
    b.observacion_no_completado AS observacion_no_completado,
    b.nom_cliente_servicio AS nom_cliente_servicio,
    b.street AS direccion,
    b.inicio_de_sla AS inicio_de_sla,
    b.fin_de_sla AS fin_de_sla,
    b.serie_medidor AS serie_medidor,
    c.default_code AS codigo_producto,
    c.name AS nombre_producto,
    a.valor AS valor,
    b.estado_cliente AS estado_cliente,
    b.etapa_id AS etapa_id,
    to_char(fecha_inicio,'YYYY/MM/DD') AS fecha
  from hsg_orden_servicios_digitados a
  LEFT JOIN hsg_orden b ON a.orden_id = b.id
  LEFT JOIN product_template c ON a.producto_id = c.id
  LEFT JOIN hsg_tipo_orden d ON a.tipo_id = d.id
  LEFT JOIN hsg_sector e ON b.sector_id = e.id
  LEFT JOIN hsg_brigadas f ON b.brigada_id = f.id
  WHERE
    to_char(fecha_inicio,'YYYY-MM-DD')>='${fechaInicio}' AND
    to_char(fecha_inicio,'YYYY-MM-DD') <='${fechaFin}' AND
    e.id IN (${
      Array.isArray(sede)
        ? sede.map((value) => `${value.split("-")[0]}`)
        : sede.split("-")[0]
    })
  order by sector asc, fecha_inicio asc;
  `);

  return orders[0];
};

export const findByConsumer = async (fechaInicio, fechaFin, sede) => {
  const ordersSeriado = await sequelize.query(`
  SELECT
    to_char(b.fecha_inicio, 'DD/MM/YYYY') AS fecha_ejecucion,
    CASE WHEN
      subconsulta.numero_serie IS NULL
      THEN b.codigo_orden
      ELSE subconsulta.numero_serie
      END AS documento,
    b.cod_cuenta AS id,
    b.codigo_orden AS num_ot,
    c.default_code AS codigo_material,
    h.name AS descripcion,
    a.numero_serie AS serie,
    a.cantidad AS cantidad,
    b.serie_medidor AS medidor,
    s.name AS sector
  FROM hsg_orden_seriados a
  INNER JOIN hsg_orden b ON a.orden_id = b.id
  INNER JOIN product_product c ON a.producto_id = c.id
  INNER JOIN product_template h ON c.product_tmpl_id = h.id
  INNER JOIN hsg_sector s ON b.sector_id = s.id
  LEFT JOIN (
    SELECT
      e.codigo_orden,
      d.numero_serie
    FROM  hsg_orden_seriados d
    INNER JOIN hsg_orden e ON d.orden_id = e.id
    INNER JOIN product_product f ON d.producto_id = f.id
    WHERE
      f.default_code in ('16134', '16135')
    ) AS subconsulta
  ON b.codigo_orden = subconsulta.codigo_orden
  WHERE
    b.sector_id IN (${
      Array.isArray(sede)
        ? sede.map((value) => `${value.split("-")[0]}`)
        : sede.split("-")[0]
    }) AND
    to_char(b.fecha_inicio,'YYYY-MM-DD') >='${fechaInicio}' AND
    to_char(b.fecha_inicio,'YYYY-MM-DD') <='${fechaFin}' AND
    c.default_code not in ('16134', '16135')
  ORDER BY b.codigo_orden ASC, sector ASC;
  `);

  const ordersNoSeriado = await sequelize.query(`
  SELECT
    to_char(b.fecha_inicio, 'DD/MM/YYYY') AS fecha_ejecucion,
    CASE WHEN
      subconsulta.numero_serie IS NULL
      THEN b.codigo_orden
      ELSE subconsulta.numero_serie
      END AS documento,
    b.cod_cuenta AS id,
    b.codigo_orden AS num_ot,
    c.default_code AS codigo_material,
    h.name AS descripcion,
    NULL AS serie,
    a.cantidad AS cantidad,
    b.serie_medidor AS medidor,
    s.name AS sector
  FROM hsg_orden_productos a
  INNER JOIN hsg_orden b ON a.orden_id = b.id
  INNER JOIN product_product c ON a.producto_id = c.id
  INNER JOIN product_template h ON c.product_tmpl_id = h.id
  INNER JOIN hsg_sector s ON b.sector_id = s.id
  LEFT JOIN (
    SELECT
      e.codigo_orden,
      d.numero_serie
    FROM  hsg_orden_seriados d
    INNER JOIN hsg_orden e ON d.orden_id = e.id
    INNER JOIN product_product f ON d.producto_id = f.id
    WHERE
      f.default_code IN ('16134', '16135')
  ) AS subconsulta
  ON b.codigo_orden = subconsulta.codigo_orden
  WHERE
    b.sector_id IN (${
      Array.isArray(sede)
        ? sede.map((value) => `${value.split("-")[0]}`)
        : sede.split("-")[0]
    }) AND
    to_char(b.fecha_inicio,'YYYY-MM-DD') >='${fechaInicio}' AND
    to_char(b.fecha_inicio,'YYYY-MM-DD') <='${fechaFin}' AND
    c.default_code NOT IN ('16134', '16135')
  ORDER BY b.codigo_orden ASC , sector ASC;
  `);

  const data = ordersSeriado[0].concat(ordersNoSeriado[0]);

  return data;
};

export const findSector = async () => {
  const sector = await sequelize.query(`
    SELECT
      id,
      name
    from hsg_sector
  `);

  return sector;
};
